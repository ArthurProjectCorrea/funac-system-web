...
- Associação usuário ↔ posições: tabela `role_profile` (um usuário pode ter múltiplas posições).
- Permissões: tabelas `screen`, `permission`, `access` (join entre `position`, `screen`, `permission`).
...
- `department` — departamentos (id, name).
- `position` — posições/cargos (id, department_id, name).
- `profile` — (agora simplificado) era ligação 1:1 com `auth.users`, mas posições agora ficam em `role_profile`.
- `role_profile` — join: `user_id` (uuid) ↔ `position` (integer).
- `screen` — recursos/navegation entries: `id`, `name`, `url`, `icon`, `sidebar` (boolean), `group_id`.
- `group_screen` — grupos de telas usados para agrupar itens da sidebar.
- `permission` — nomes de ações, por exemplo `view`, `create`, etc.
- `access` — join entre `position_id`, `screen_id`, `permission_id` que determina quais posições têm determinada permissão sobre determinada tela.

As migrações relevantes estão em `supabase/migrations/` (ver `20260221153621_create_user_permision.sql`, `20260222123000_update_screen_table.sql`, `20260222124500_add_group_screen_and_screen_cols.sql`, `20260221172000_extract_roles_to_table.sql`).
...
3. Se `user` existir:
   - Carrega `profile` (se existir) via `profile.user_id = user.id`.
   - Carrega posições do usuário via `role_profile` (coluna `position`).
   - Carrega linhas completas de `position` para essas posições.
   - Carrega `access` para essas posições.
   - Carrega `screen` e `permission` referenciados por `access`.
   - Carrega `group_screen` para agrupar sidebar.
...
4. Em seguida faz uma query em `access` procurando qualquer row onde `position_id` esteja entre as posições do usuário, `screen_id` = screen.id e `permission_id` = viewPerm.id. Se não existe nenhum `access` correspondente, chama `forbidden()`.
...
- `AppSidebar` consome `authData` e monta a navegação agrupando `screens` por `group_screen` e filtrando por `sidebar = true`.
...
2. Seed (exemplo idempotente SQL) que adiciona um `screen` `/teste`, uma `permission` `create` e registra `access` para as posições do usuário alvo:
...
WHERE NOT EXISTS (SELECT 1 FROM permission WHERE name = 'create');

INSERT INTO screen (name, url, sidebar, group_id, created_at, updated_at)
SELECT 'Teste', '/teste', true, NULL, now(), now()
WHERE NOT EXISTS (SELECT 1 FROM screen WHERE url = '/teste');

INSERT INTO access (position_id, screen_id, permission_id, created_at, updated_at)
SELECT rp.position, s.id, p.id, now(), now()
FROM role_profile rp
JOIN screen s ON s.url = '/teste'
JOIN permission p ON p.name = 'create'
LEFT JOIN access a ON a.position_id = rp.position AND a.screen_id = s.id AND a.permission_id = p.id
...
- A fonte da verdade para permissões é a tabela `access`. A lógica atual exige explicitamente a permissão `view` para que uma rota seja exibida.
- Se quiser ajustes de UX: em vez de negar por padrão quando `permission.view` não existe, a equipe pode escolher permitir por padrão e somente negar quando `permission.view` existe mas não há `access` (política permissiva vs. política restritiva).
